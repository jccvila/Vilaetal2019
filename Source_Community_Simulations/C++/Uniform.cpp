...
public:
    ...
    void mutate(int index)
    {
        ...
        {
            double fitnesseffect = rng.d01();
            // 50-50 change of up or down
            if (rng.d01() > 0.5)
            {
                    // do stuff to population.
                    if(population[index] == max_cat){max_cat += fitnesseffect;}
                    if(population[index] == min_cat && type_abundances[types[index]] == 1){check_min_cat = true;}
                    population[index] += fitnesseffect;
            }

            else
            {
                    // do opposite to population
                    if(population[index] == min_cat){min_cat -= fitnesseffect;}
