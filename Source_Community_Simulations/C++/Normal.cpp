...
public:
    ...
    // first algorithm works with integer shape use multiplication method to get sum of exponential distribution
    double gm(int shape)
    {
        double i = 0; // correcting because do while loop updates i before it checks if i = shape.
        double p = 1;
        double u;
        do{
            u = d01();
    ...
    // second algorithm with fractional shape (shape between 0 and 1)
    double gt(double shape)
    {
        double b;
        double p;
        double x;
        ...
        do{
            ...
            x = -log((b-p)/shape);
            u =d01();
            if(u > pow(x,shape-1)){restart = true;}
            else{restart = false;}
            }
        }
        while(restart);
    ...
    // use two algorithm obtained from ahrens and dieter relies on fact that
    // If x and y are independent gamma-distributed random variables with parameters a and b respectively then x+ y is gamma-distributed with parameter a + b.
    double gamma(double shape,double scale)
    {
        double y;
        double z;
        int m = floor(shape) ;// integer part
        double f = shape - m ; // fractional part
        if (m != 0){y = gm(m);}
        else{y =0;}
        if (f != 0){z = gt(f);}
        else{z = 0;}
...
private:
    ...
    // populations
    vector<double> home_population;  // starting population.
    vector<long> home_types;
    vector<long> home_type_abundances;
    vector<double> home_type_categories;
    ...
    vector<vector<double> > vectors_of_doubles;
    ...
    vector<vector<double> >  vector_of_type_categories; // vector of type categories
...
public:
    ...
    double fitness_calculator(double fitness_category)
    {
    ...
    void mutate(int index)
    {
        ...
        {
            double fitnesseffect = rng.norm();
            // 50-50 change of up or down
            // do stuff to population.
            double newfitness = population[index] + fitnesseffect;
            if(newfitness >= max_cat){max_cat = newfitness;}
            if(population[index] == max_cat){check_max_cat = true;}
            if(newfitness <= min_cat){min_cat = newfitness;}
            if(population[index] == min_cat){check_min_cat = true;}
            population[index] = newfitness;
...
//               cout << limit << endl;
    ...
    void runsimulation(long generations, long mean_interval, long variance_interval, long abundances_interval)
    {
        ...
        vectors_of_doubles =  vector_of_type_categories;
        savevectordoubles(categories_path);
        vectors_of_doubles.clear();
