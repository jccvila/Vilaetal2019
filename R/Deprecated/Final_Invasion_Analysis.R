...
Invasion_Success <- function(seed,task){
  invfreq = read_data_frequency(seed,task)
  invsuccess <- c()
  for(i in unique(invfreq$Run)){
    tempdata = subset(invfreq,Run == i)
    if(nrow(tempdata) >= 1000){
      invsuccess <- c(invsuccess,sum(tempdata$Invasion_Frequency[1:1000]))
    }
    if(nrow(tempdata) < 1000){
      invsuccess <- c(invsuccess,sum(tempdata$Invasion_Frequency[1:nrow(tempdata)]) + tempdata$Invasion_Frequency[nrow(tempdata)] * (1000 - nrow(tempdata)))
    }
  }
  return(invsuccess)
}
...
read_data_summary<- function(seed,task){
  ...
  # tmydf$invador_popsize = paramaters$Value[8]
  # tmydf$invador_generations = paramaters$Value[9]
  return(tmydf)
}
...
for(j in 1:270){
for (i in 1:100){
  Success <- Invasion_Success(j,i)
  Paramaters <- t(replicate(length(Success), read_data_paramaters(j,i)[,2]))
  colnames(Paramaters) <- read_data_paramaters(j,i)[,1]
  Other <- read_data_summary(j,i)
  tempplotdata = cbind(Paramaters,Other,Success)
  plotdata <- rbind(plotdata,tempplotdata)
  rownames(plotdata) <- NULL
}
}
save(plotdata,'Additive_Data.Rda')

plotdata$Relative_Fitness = plotdata$Invader_Mean_End/plotdata$Invader_Mean_End
# generic ### 
...
sum_stat_Invasion_Frequency <- function(Invador_popsize,Invador_generations,Mutation_rate,Selection_strength,Propagule_pressure,Home_popsize){
...
# calculates mean Invasion_Frequency for first 1000 generations
...
proportion_calculator <- function(data){
  ...
  n <- nrow(EndsimData)
  proportion  <- sum(EndsimData$Invasion_Frequency)/n
...
success_calculator <- function(data){
  ...
  mean <- aggregate(Invasion_Frequency ~  Generation, data = data, FUN = function (x) mean(x))
  min <- aggregate(Invasion_Frequency ~  Generation, data = data, FUN = function (x) min(x))
  max <- aggregate(Invasion_Frequency ~  Generation, data = data, FUN = function (x) max(x))
  if(big == TRUE){
    mean <- mean[1:1000,]
    min <- min[1:1000,]
    max <- max[1:1000,]
  ...
  return(data.frame(Mean = sum(mean$Invasion_Frequency), Max = sum(max$Invasion_Frequency), Min = sum(min$Invasion_Frequency)))
}
...
gradient_calculator <- function(data){
  ...
  min <- min[1:2,]
  max <- max[1:2,]
  return(data.frame(Mean = mean$Invasion_Frequency[2] - mean$Invador_Frequenc[1], Max = max$Invasion_Frequency[2] - max$Invador_Frequenc[1],Min = min$Invasion_Frequency[2] - min$Invador_Frequenc[1]))
}
...
invfreqvstime <- function(Invador_popsize,Invador_generations,Mutation_rate,Selection_strength,Propagule_pressure,Home_popsize){
  df_freq <- sum_stat_Invasion_Frequency(Invador_popsize,Invador_generations,Mutation_rate,Selection_strength,Propagule_pressure,Home_popsize)
...
invfreqvstime2 <- function(Invador_popsize,Invador_generations,Mutation_rate,Selection_strength,Propagule_pressure,Home_popsize){
  data <- sum_stat_Invasion_Frequency(Invador_popsize,Invador_generations,Mutation_rate,Selection_strength,Propagule_pressure,Home_popsize)
  mean <- aggregate(Invasion_Frequency ~  Generation, data = data, FUN = function (x) mean(x))
  min <- aggregate(Invasion_Frequency ~  Generation, data = data, FUN = function (x) min(x))
  max <- aggregate(Invasion_Frequency ~  Generation, data = data, FUN = function (x) max(x))
  df_freq = data.frame(Generation = mean$Generation, Mean = mean$Invasion_Frequency, Min = min$Invasion_Frequency, Max = max$Invasion_Frequency)
...
Internalplots <- function(Mutation_rate,Selection_strength,Invador_generations,Invador_popsize,Home_popsize,Propagule_pressure){
  ...
  aggregated <- aggregate(Invasion_Frequency ~  Run, data = SortedDF, FUN = function (x) sum(x))
  plotdf <- rbind(plotdf,cbind(aggregated,Richness,Relative_Fitness,Propagule_pressure,Adaptation_rate,Richness))
  }
  p1 <- ggplot(plotdf, aes(x = Relative_Fitness, y = Invasion_Frequency,color = as.factor(Propagule_pressure))) +  geom_point(size=3)  + 
  ...
  for (j in listhomepop){
    ...
    Adaptation_rate <- (read_data_end_fitness(paramsdf2$Seed,paramsdf2$Task)/read_data_start_fitness(paramsdf2$Seed,paramsdf2$Task))/read_data_paramaters(201,1)[[9,2]]
    aggregated <- aggregate(Invasion_Frequency ~  Run, data = SortedDF, FUN = function (x) sum(x))
    gradient <- summary(lm(aggregated$Invasion_Frequency~Richness))$coefficients[[2]]
  ...
  p3 <- ggplot(plotdf, aes(x = Richness, y = Invasion_Frequency,color = as.factor(Home_popsize))) +  geom_point(size=3)  + 
...
richness_coefficient_calculator <- function(seed,task){
  ...
  for(j in unique(Invasion_freq_df$Run)){
    ...
    if(subset_size >= 1000){
      sum_freq <- sum(subset$Invasion_Frequency[1:1000])
      Invasion_Outcome <- c(Invasion_Outcome,sum_freq)
    }
    else{
      sum_freq <- sum(subset$Invasion_Frequency[1:subset_size]) + sum(rep(subset$Invasion_Frequency[subset_size],1000 - subset_size))
      Invasion_Outcome <- c(Invasion_Outcome, sum_freq)
    }
  }
