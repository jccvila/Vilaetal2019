...
IR = c()
IRRS = c()
plotdata[,PP:=slope(propagule_pressure,Frequency400),by=list(selection_strength,mutation_rate,invaded_popsize,invader_popsize,invader_generations)]
plotdata[,PPRS:=rsquared(propagule_pressure,Frequency400),by=list(selection_strength,mutation_rate,invaded_popsize,invader_popsize,invader_generations)]
plotdata[,PR:=slope(Propagule_Richness,Frequency400),by=list(selection_strength,mutation_rate,propagule_pressure,invaded_popsize,invader_popsize,invader_generations)]
plotdata[,PRRS:=rsquared(Propagule_Richness,Frequency400),by=list(selection_strength,mutation_rate,propagule_pressure,invaded_popsize,invader_popsize,invader_generations)]
plotdata[,IS:=slope(invaded_popsize,Frequency400),by=list(selection_strength,mutation_rate,propagule_pressure,invader_popsize,invader_generations)]
plotdata[,ISRS:=rsquared(invaded_popsize,Frequency400),by=list(selection_strength,mutation_rate,propagule_pressure,invader_popsize,invader_generations)]
plotdata[,IR:=slope(Invaded_Richness_Start,Frequency400),by=list(selection_strength,mutation_rate,propagule_pressure,invaded_popsize,invader_popsize,invader_generations)]
plotdata[,IRRS:=rsquared(Invaded_Richness_Start,Frequency400),by=list(selection_strength,mutation_rate,propagule_pressure,invaded_popsize,invader_popsize,invader_generations)]
...
#   PP = c(PP,slope(t1$propagule_pressure,t1$Frequency400))
#   PPRS =c(PPRS,rsquared(t1$propagule_pressure,t1$Frequency400))
#   PR=c(PR,slope(t3$Propagule_Richness,t3$Frequency400))
#   PRRS=c(rsquared(t3$Propagule_Richness,t3$Frequency400))
#   IS = c(IS,slope(t2$invaded_popsize,t2$Frequency400))
#   ISRS = c(ISRS,rsquared(t2$invaded_popsize,t2$Frequency400))
#   IR = c(IR,slope(t3$Invaded_Richness_Start,t3$Frequency400))
#   IRRS = c(IRRS,rsquared(t3$Invaded_Richness_Start,t3$Frequency400))
# }
#
# #
# # # function takes abundance vector and converts it into popsize.
save(plotdata,file = 'Additive_Data3_Supp.rds')
stop()
load('Additive_Data3_Supp.rds')
s = unique(plotdata$selection_strength)
u = unique(plotdata$mutation_rate)
p = unique(plotdata$propagule_pressure)
i <- unique(plotdata$invaded_popsize)
...
plot_Invasion_Frequency <- function(seed,task,run){
  invaderFrequency <- read_data_frequency(seed,task)
  subset <- invaderFrequency[which(invaderFrequency$Run == run),]
  plot(subset$Generation,subset$Invasion_Frequency,type="l",ylim=c(0,1))
}
...
invfreqvstime <- function(a,b,c,d,e,f){
  df_freq <- param_inv_freq(a,b,c,d,e,f)
  df_freq <- melt(df_freq,id.vars='Generation',variable.name = "Run",value.name = "Invasion_Frequency")
  p1 <- ggplot(df_freq, aes(x = Generation, y = Invasion_Frequency,col=as.factor(Run))) + geom_line(size=1) + ylab("Invasion_Frequency") + theme_classic() + 
    ...
    xlim(0,1000) + ylim(0,1) +  theme(text = element_text(size=26)) + ylab("Invader Frequency") + xlim(0,300)
  # ggsave(filename = "../../Plots/InvasionModel/OneRun.pdf",p1)
  return(p1)
}

invfreqvstime2 <- function(a,b,c,d,e,f){
  df_freq <- param_inv_freq(a,b,c,d,e,f)
  runs <- ncol(df_freq)-1
  df_freq <- data.frame(Variance = apply(df_freq[,-which(names(df_freq) == "Generation")],1,var),
                        Mean = apply(df_freq[,-which(names(df_freq) == "Generation")],1,mean), 
                        Generation = df_freq[,which(names(df_freq) == "Generation")])
  df_freq$SE = sqrt(df_freq$Variance/runs)
  df_freq$Upper_CI = pmin(df_freq$Mean + (1.96 * df_freq$SE),1)
  df_freq$Lower_CI = pmax(df_freq$Mean - (1.96 * df_freq$SE),0)
  p1 <- ggplot(df_freq, aes(x = Generation, y = Mean)) + geom_line(size=1) + ylab("Mean Fitness Category") + theme_classic()+ 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+
    theme(panel.border= element_blank()) + theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1)) + 
    geom_ribbon(aes(ymin = Upper_CI, ymax = Lower_CI), alpha = 0.5) + ylim(0,1)
  return(p1)
}

Invasionvsgenerations <- function(a,b,c,d,e){
  row1 = which(plotdata$selection_strength==s[a] & 
                 plotdata$mutation_rate == u[b] &
                 plotdata$propagule_pressure == p[c] &
               plotdata$invaded_popsize == i[f])
  plotdf1 <- plotdata[row1]

  # plotdf <- aggregate(Success ~ invader_generations + invaded_popsize,data = plotdf, function(x) c(M = mean(x), SE = sd(x)/sqrt(length(x))))
  # plotdf <- data.frame(invader_generations = plotdf$invader_generations,invaded_popsize = plotdf$invaded_popsize, Mean = plotdf$Success[,1],Upper_CI = plotdf$Success[,1] + 1.96 * plotdf$Success[,2],Lower_CI = plotdf$Success[,1]- 1.96 * plotdf$Success[,2])
  plotdf1 <- aggregate(frequency40 ~ invader_generations + invaded_popsize, plotdf1, mean)
  p1 <- ggplot(plotdf1, aes(x = invader_generations, y = Success,color = as.factor(invaded_popsize))) +  geom_point(size=2) + geom_line(size=1) + 
    ...
    labs(color=expression("J"[d])) + scale_x_log10() + scale_y_continuous(limits = c(0,1000))  +
    theme(text = element_text(size=18),legend.text = element_text(size=15),legend.key.size = unit(0.5, "cm"))  
  row2 = which(plotdata$selection_strength==s[a] & 
                 plotdata$mutation_rate == u[b] &
                 plotdata$propagule_pressure == p[c] &
                 plotdata$invaded_popsize == i[d])
  ...
  # plotdf <- aggregate(Success ~ invader_generations + invaded_popsize,data = plotdf, function(x) c(M = mean(x), SE = sd(x)/sqrt(length(x))))
  # plotdf <- data.frame(invader_generations = plotdf$invader_generations,invaded_popsize = plotdf$invaded_popsize, Mean = plotdf$Success[,1],Upper_CI = plotdf$Success[,1] + 1.96 * plotdf$Success[,2],Lower_CI = plotdf$Success[,1]- 1.96 * plotdf$Success[,2])
  plotdf2 <- aggregate(Success ~ invader_generations + invader_popsize, plotdf2, mean)
  p2 <- ggplot(plotdf2, aes(x = invader_generations, y = Success,color = as.factor(invader_popsize))) +  geom_point(size=2) + geom_line(size=1) + 
    xlab(expression( "Invader Generations g"[r]) ) +ylab("Success") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) +
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color=expression("J"[r])) + scale_x_log10() + scale_y_continuous(limits = c(0,1000))  +
    theme(text = element_text(size=18),legend.text = element_text(size=15),legend.key.size = unit(0.5, "cm"))  
  row3 = which(plotdata$selection_strength==s[a] & 
                 plotdata$mutation_rate == u[b] &
                 plotdata$invader_popsize == r[e] &
                 plotdata$invaded_popsize == i[d])
  plotdf3 <- plotdata[row3]
  # plotdf <- aggregate(Success ~ invader_generations + invaded_popsize,data = plotdf, function(x) c(M = mean(x), SE = sd(x)/sqrt(length(x))))
  # plotdf <- data.frame(invader_generations = plotdf$invader_generations,invaded_popsize = plotdf$invaded_popsize, Mean = plotdf$Success[,1],Upper_CI = plotdf$Success[,1] + 1.96 * plotdf$Success[,2],Lower_CI = plotdf$Success[,1]- 1.96 * plotdf$Success[,2])
  plotdf3 <- aggregate(Success ~ invader_generations + propagule_pressure, plotdf3, mean)
  p3 <- ggplot(plotdf3, aes(x = invader_generations, y = Success,color = as.factor(propagule_pressure))) +  geom_point(size=2) + geom_line(size=1) + 
    xlab(expression( "Invader Generations g"[r]) ) +ylab("Success") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) + scale_y_continuous(limits = c(0,1000))  +
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color=expression("P")) + scale_x_log10()  +
    theme(text = element_text(size=18),legend.text = element_text(size=15),legend.key.size = unit(0.5, "cm"))  
  return(list(p1,p2,p3))
}

Invasionvspopsize <- function(a,b,c,d,f){
  row1 = which(plotdata$selection_strength==s[a] & 
                 plotdata$mutation_rate == u[b] &
                 plotdata$propagule_pressure == p[c] &
                 plotdata$invader_generations == g[f])
  ...
  # plotdf <- aggregate(Success ~ invader_generations + invaded_popsize,data = plotdf, function(x) c(M = mean(x), SE = sd(x)/sqrt(length(x))))
  # plotdf <- data.frame(invader_generations = plotdf$invader_generations,invaded_popsize = plotdf$invaded_popsize, Mean = plotdf$Success[,1],Upper_CI = plotdf$Success[,1] + 1.96 * plotdf$Success[,2],Lower_CI = plotdf$Success[,1]- 1.96 * plotdf$Success[,2])
  plotdf1 <- aggregate(Success ~ invader_popsize + invaded_popsize, plotdf1, mean)
  p1 <- ggplot(plotdf1, aes(x = invader_popsize, y = Success,color = as.factor(invaded_popsize))) +  geom_point(size=2) + geom_line(size=1) + 
  ...
  # plotdf <- data.frame(invader_generations = plotdf$invader_generations,invaded_popsize = plotdf$invaded_popsize, Mean = plotdf$Success[,1],Upper_CI = plotdf$Success[,1] + 1.96 * plotdf$Success[,2],Lower_CI = plotdf$Success[,1]- 1.96 * plotdf$Success[,2])
  plotdf2 <- aggregate(Success ~ invader_generations + invader_popsize, plotdf2, mean)
  p2 <- ggplot(plotdf2, aes(x = invader_popsize, y = Success,color = as.factor(invader_generations))) +  geom_point(size=2) + geom_line(size=1) + 
    xlab(expression( "Invader Community Size J"[r]) ) +ylab("Success") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) +  geom_vline(xintercept = i[d],linetype=2,col='red') +
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color=expression("G"[r])) + scale_x_log10() + scale_y_continuous(limits = c(0,1000))  +
    theme(text = element_text(size=18),legend.text = element_text(size=15),legend.key.size = unit(0.5, "cm"))  
  row3 = which(plotdata$selection_strength==s[a] & 
                 plotdata$mutation_rate == u[b] &
                 plotdata$invader_generations== g[f] &
  ...
  # plotdf <- aggregate(Success ~ invader_generations + invaded_popsize,data = plotdf, function(x) c(M = mean(x), SE = sd(x)/sqrt(length(x))))
  # plotdf <- data.frame(invader_generations = plotdf$invader_generations,invaded_popsize = plotdf$invaded_popsize, Mean = plotdf$Success[,1],Upper_CI = plotdf$Success[,1] + 1.96 * plotdf$Success[,2],Lower_CI = plotdf$Success[,1]- 1.96 * plotdf$Success[,2])
  plotdf3 <- aggregate(Success ~ invader_popsize + propagule_pressure, plotdf3, mean)
  p3 <- ggplot(plotdf3, aes(x = invader_popsize, y = Success,color = as.factor(propagule_pressure))) +  geom_point(size=2) + geom_line(size=1) + 
    xlab(expression( "Invader Community Size J"[r]) ) +ylab("Success") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) + scale_y_continuous(limits = c(0,1000))  +
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
...
Invasionvspropagulepressure <- function(a,b,d,e,f){
  row1 = which(plotdata$selection_strength==s[a] & 
                 plotdata$mutation_rate == u[b] &
                 plotdata$invader_popsize == r[e] &
                 plotdata$invader_generations == g[f])
  ...
  # plotdf <- aggregate(Success ~ invader_generations + invaded_popsize,data = plotdf, function(x) c(M = mean(x), SE = sd(x)/sqrt(length(x))))
  # plotdf <- data.frame(invader_generations = plotdf$invader_generations,invaded_popsize = plotdf$invaded_popsize, Mean = plotdf$Success[,1],Upper_CI = plotdf$Success[,1] + 1.96 * plotdf$Success[,2],Lower_CI = plotdf$Success[,1]- 1.96 * plotdf$Success[,2])
  plotdf1 <- aggregate(Success ~ propagule_pressure + invaded_popsize, plotdf1, mean)
  p1 <- ggplot(plotdf1, aes(x = propagule_pressure, y = Success,color = as.factor(invaded_popsize))) +  geom_point(size=2) + geom_line(size=1) + 
    xlab(expression( "Propagule Pressure P") ) +ylab("Success") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank())  +
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color=expression("J"[d])) + scale_x_log10() + scale_y_continuous(limits = c(0,1000))  +
  ...
  # plotdf <- aggregate(Success ~ invader_generations + invaded_popsize,data = plotdf, function(x) c(M = mean(x), SE = sd(x)/sqrt(length(x))))
  # plotdf <- data.frame(invader_generations = plotdf$invader_generations,invaded_popsize = plotdf$invaded_popsize, Mean = plotdf$Success[,1],Upper_CI = plotdf$Success[,1] + 1.96 * plotdf$Success[,2],Lower_CI = plotdf$Success[,1]- 1.96 * plotdf$Success[,2])
  plotdf2 <- aggregate(Success ~ invader_generations + propagule_pressure, plotdf2, mean)
  p2 <- ggplot(plotdf2, aes(x = propagule_pressure, y = Success,color = as.factor(invader_generations))) +  
  ...
  # plotdf <- data.frame(invader_generations = plotdf$invader_generations,invaded_popsize = plotdf$invaded_popsize, Mean = plotdf$Success[,1],Upper_CI = plotdf$Success[,1] + 1.96 * plotdf$Success[,2],Lower_CI = plotdf$Success[,1]- 1.96 * plotdf$Success[,2])
  plotdf3 <- aggregate(Success ~ invader_popsize + propagule_pressure, plotdf3, mean)
  p3 <- ggplot(plotdf3, aes(x = propagule_pressure, y = Success,color = as.factor(invader_popsize))) +  geom_point(size=2) + geom_line(size=1) + 
    xlab(expression( "Propagule Pressure P") ) +ylab("Success") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) + scale_y_continuous(limits = c(0,1000))  +
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color=expression("J"[r])) + scale_x_log10()  +
    theme(text = element_text(size=18),legend.text = element_text(size=15),legend.key.size = unit(0.5, "cm"))  
  return(list(p1,p2,p3))
}
...
Time_series_plot <- function(){
  ...
  ggsave(file = '../../Plots/Timeseries_A_supp.png', grid.arrange(arrangeGrob(p1),arrangeGrob(p2),arrangeGrob(p3),arrangeGrob(p4),arrangeGrob(p5),arrangeGrob(p6),arrangeGrob(p7),
...
Evolution_Plots <- function(a,b,d){
  ...
  sum_df2  =plotdf1[,list(mean=mean(Frequency400),sd=sd(Frequency400)),by=list(invader_popsize,invader_generations,propagule_pressure)]
  ...
  p2 <- ggplot(plotdf1, aes(x = Adaptation_Rate_Invader, y = Relative_Propagule_Fitness,color = as.factor(invader_generations))) + 
    ...
    labs(color='Generations') + 
    theme(text = element_text(size=18),legend.text = element_text(size=15),legend.key.size = unit(0.5, "cm")) 
  p3 <- ggplot(plotdf1, aes(x = Relative_Propagule_Fitness, y =Frequency400,color = as.factor(invader_generations))) +  
    geom_point(size=0.5) + 
  ...
  ggsave(file = '../../Plots/Evolution_A_supp.png',grid.arrange(p1,p2,p3,p4,leg
...
Invader_Plots <- function(a,b,d){
  ...
  sum_df  =tbdf[,list(mean=mean(Frequency400),sd=sd(Frequency400)),by=list(invader_popsize,invader_generations,propagule_pressure,invaded_popsize)]
  ...
  p2 <- ggplot(sum_df2, aes(x = Relative_Propagule_Fitness,y = mean)) +  geom_point(size=2) + 
    ...
    theme(text = element_text(size=18),legend.text = element_text(size=15),legend.key.size = unit(0.5, "cm")) 
  sum_df3  =tbdf[,list(mean=mean(Frequency400),sd=sd(Frequency400)) ,by=list(invader_popsize,invader_generations,Propagule_Richness,propagule_pressure,invaded_popsize)]
  ...
  p4 <- ggplot(sum_df4, aes(x = Relative_Propagule_Fitness,y = mean)) +  geom_point(size=2) + 
    ...
    labs(color='Invaded Community Size') + 
    theme(text = element_text(size=18),legend.text = element_text(size=15),legend.key.size = unit(0.5, "cm")) 
  ggsave(file = '../../Plots/Invaders_A_supp.png',grid.arrange(arrangeGrob(p1),arrangeGrob(p2),arrangeGrob(p3),arrangeGrob(p4)
...
#   options("scipen"=10) 
# 
#   ggsave(file = '../../Plots/Invaded_A_supp.png',grid.arrange(arrangeGrob(p1),arrangeGrob(p2),arrangeGrob(p3),arrangeGrob(p4)
#                                                           ,layout_matrix = rbind(c(1,2),c(3,4))),width = 12, height = 10)
#   
#   return( list(p1,p2))
# }
...
Recreation_plots <- function(){
  ...
  p2a <-  ggplot(ag[ag$Generation%in%c(1,100,200),],aes(x = Generation,y=mean,colour=Treatment)) + 
    ...
    labs(x = 'Time(Generation)',y =expression(atop('Invader Success', '(log'[10]*'frequency)')),colour='P') +
    ...
    theme(legend.position = c(0.8,0.6),legend.text = element_text(size=12),legend.key.size = unit(0.5, "cm"),legend.margin=margin(t=-0.1, r=-0.1, b=-0.1, l=-0.1, unit="cm")) +
    theme(text = element_text(size=15),axis.text = element_text(size=15))
  ...
  ggsave(file = '../../Plots/Recreation_A_supp.png',grid.arrange(arrangeGrob(p1a,top=textGrob('A)',x=0.05,gp=gpar(fontsize=15))),
                                                            arrangeGrob(p2a,top=textGrob('C)',x=0.05,gp=gpar(fontsize=15))),
                                                            arrangeGrob(p1b,top=textGrob('B)',x=0.05,gp=gpar(fontsize=15))),
                                                            arrangeGrob(p2b,top=textGrob('D)',x=0.05,gp=gpar(fontsize=15))),ncol=2),
                                                            width = 12, height = 10)
}           
# 
