...
###read _data
path = "../../Results/Invasion_submission/Type_Invasion/"
...
#       possibleError <- tryCatch(read_data_frequency(seed,task),error = function(e) e)
      ...
      Propagule_richness <- params[12,2]
  ...
  paramatersdf$Relative_Fitness <- paramatersdf$Propagule_pressure/paramatersdf$Home_popsize
...
sum_stat_invador_frequency <- function(Invador_popsize,Invador_generations,Mutation_rate,Selection_strength,Propagule_pressure,Home_popsize){
  row = which(paramatersdf$Invador_popsize == Invador_popsize & paramatersdf$Invador_generations == Invador_generations & paramatersdf$Mutation_rate == Mutation_rate & paramatersdf$Selection_strength == Selection_strength & paramatersdf$Propagule_pressure == Propagule_pressure & paramatersdf$Home_popsize == Home_popsize)
  seed = paramatersdf$Seed[row]
  task = paramatersdf$Task[row]
...
Invasionvsgenerations <- function(Invador_popsize,Mutation_rate,Selection_strength,Propagule_pressure){
  plotdf = finaldf[which(paramatersdf$Invador_popsize  == Invador_popsize & paramatersdf$Mutation_rate == Mutation_rate & paramatersdf$Selection_strength == Selection_strength & paramatersdf$Propagule_pressure == Propagule_pressure),]
  p1 <- ggplot(plotdf, aes(x = Invador_generations, y = Proportion,color = as.factor(Home_popsize))) +  geom_point(size=2) + geom_line(size=1) + 
    xlab("Invador Generations") +ylab("Proportion Succesful Invasion") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) + 
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color='Home population size') # +  geom_ribbon(aes(ymin = Upper_CI, ymax = Lower_CI,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
#   p2 <- ggplot(plotdf, aes(x = Invador_generations, y = Gradient ,color = as.factor(Home_popsize))) +  geom_point(size=2) + geom_line(size=1) + 
#     xlab("Invador Generations") +ylab("Gradient") + theme_classic() + 
#     theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
#     theme(panel.border= element_blank()) + 
#     theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
#     labs(color='Home population size') # +  geom_ribbon(aes(ymin = Min, ymax = Max,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
  p2 <- ggplot(plotdf, aes(x = Invador_generations, y = Invasion_success,color = as.factor(Home_popsize))) +  geom_point(size=2) + geom_line(size=1) + 
    xlab("Invador Generations") +ylab("Success") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) + 
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color='Home population size') # +  geom_ribbon(aes(ymin = Min, ymax = Max,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
  # + geom_ribbon(aes(ymin = Min, ymax = Max,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
  return(list(p1,p2))
}

Invasionvspopsize <- function(Invador_generations,Mutation_rate,Selection_strength,Propagule_pressure){
  plotdf = finaldf[which(paramatersdf$Invador_generations  == Invador_generations & paramatersdf$Mutation_rate == Mutation_rate & paramatersdf$Selection_strength == Selection_strength & paramatersdf$Propagule_pressure == Propagule_pressure),]
  p1 <- ggplot(plotdf, aes(x = Invador_popsize, y = Proportion,color = as.factor(Home_popsize))) +  geom_point(size=2) + geom_line(size=1) + 
    xlab("Invador popsize") +ylab("Proportion Succesful Invasion") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) + 
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color='Home population size') # +  geom_ribbon(aes(ymin = Upper_CI, ymax = Lower_CI,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
  p3 <- ggplot(plotdf, aes(x = Invador_popsize, y = Invasion_success,color = as.factor(Home_popsize))) +  geom_point(size=2) + geom_line(size=1) + 
    xlab("Invador popsize") +ylab("Success") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) + 
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color='Home population size') # +  geom_ribbon(aes(ymin = Min, ymax = Max,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
...
Invasionvspropagulepressure <- function(Invador_generations,Invador_popsize,Mutation_rate,Selection_strength){
  plotdf = finaldf[which(paramatersdf$Invador_generations  == Invador_generations & paramatersdf$Mutation_rate == Mutation_rate & paramatersdf$Selection_strength == Selection_strength & paramatersdf$Invador_popsize == Invador_popsize),]
  p1 <- ggplot(plotdf, aes(x = Propagule_pressure, y = Proportion,color = as.factor(Home_popsize))) +  geom_point(size=2) + geom_line(size=1) + 
    xlab("Propagule Pressure") +ylab("Proportion Succesful Invasion") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) + 
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color='Home population size') # +  geom_ribbon(aes(ymin = Upper_CI, ymax = Lower_CI,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
#   p2 <- ggplot(plotdf, aes(x = Propagule_pressure, y = Gradient,color = as.factor(Home_popsize))) +  geom_point(size=2) + geom_line(size=1) + 
#     xlab("Propagule Pressure")  +ylab("Gradient") + theme_classic() + 
#     theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
#     theme(panel.border= element_blank()) + 
#     theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
#     labs(color='Home population size') # +  geom_ribbon(aes(ymin = Min, ymax = Max,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
  p3 <- ggplot(plotdf, aes(x = Propagule_pressure, y = Invasion_success,color = as.factor(Home_popsize))) +  geom_point(size=2) + geom_line(size=1) + 
    xlab("Propagule Pressure")  +ylab("Success") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) + 
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color='Home population size') # +  geom_ribbon(aes(ymin = Min, ymax = Max,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
...
Invasionvspropaguleproportion <- function(Invador_generations,Invador_popsize,Mutation_rate,Selection_strength){
  plotdf = finaldf[which(paramatersdf$Invador_generations  == Invador_generations & paramatersdf$Mutation_rate == Mutation_rate & paramatersdf$Selection_strength == Selection_strength & paramatersdf$Invador_popsize == Invador_popsize),]
  p1 <- ggplot(plotdf, aes(x = Propagule_proportion, y = Proportion,color = as.factor(Home_popsize))) +  geom_point(size=2) + geom_line(size=1) + 
    xlab("Propagule proportion") +ylab("Proportion Succesful Invasion") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) + 
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color='Home population size') # +  geom_ribbon(aes(ymin = Upper_CI, ymax = Lower_CI,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
#   p2 <- ggplot(plotdf, aes(x = Propagule_proportion, y = Gradient,color = as.factor(Home_popsize))) +  geom_point(size=2) + geom_line(size=1) + 
#     xlab("Propagule Proportion")  +ylab("Gradient") + theme_classic() + 
#     theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
#     theme(panel.border= element_blank()) + 
#     theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
#     labs(color='Home population size') # +  geom_ribbon(aes(ymin = Min, ymax = Max,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
  p3 <- ggplot(plotdf, aes(x = Propagule_proportion, y = Invasion_success,color = as.factor(Home_popsize))) +  geom_point(size=2) + geom_line(size=1) + 
    xlab("Propagule Proportion")  +ylab("Success") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) + 
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color='Home population size') # +  geom_ribbon(aes(ymin = Min, ymax = Max,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
...
Invasionvsrelativefitness <- function(Mutation_rate,Selection_strength,Home_popsize){
  ...
  p1 <- ggplot(plotdf, aes(x = End_fitness/Start_fitness, y = Proportion,color = as.factor(Propagule_pressure))) +  geom_point(size=2)  + 
    xlab("Relative Propagule Fitness") +ylab("Proportion Succesful Invasion") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) + 
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color='Propagule Pressure') # +  geom_ribbon(aes(ymin = Upper_CI, ymax = Lower_CI,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
  p3 <- ggplot(plotdf, aes(x = End_fitness/Start_fitness, y = Invasion_success,color = as.factor(Invador_generations))) +  geom_point(size=2) + 
    xlab("Relative Propagule Fitness") +ylab("Success") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) + 
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color='Propagule Pressure') # +  geom_ribbon(aes(ymin = Min, ymax = Max,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
  # + geom_ribbon(aes(ymin = Min, ymax = Max,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
  return( list(p1,p3))
}
...
Invasionvspropagulerichness <- function(Mutation_rate,Selection_strength,Home_popsize,Propagule_pressure){
  plotdf = finaldf[which(finaldf$Mutation_rate == Mutation_rate & finaldf$Selection_strength == Selection_strength),]
  ...
  ggplot(plotdf,aes(Propagule_richness_correlation,col = as.factor(Invador_generations))) + geom_freqpoly(binwidth = 5)
  p1 <- ggplot(plotdf, aes(x = Propagule_richness_correlation, y = Proportion,color = as.factor(Propagule_pressure))) +  geom_point(size=2)  + 
    xlab("Propagule richness") +ylab("Proportion Succesful Invasion") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) + 
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color='Propagule Pressure') # +  geom_ribbon(aes(ymin = Upper_CI, ymax = Lower_CI,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
  p3 <- ggplot(plotdf, aes(x = Propagule_richness_correlation, y = Invasion_success,color = as.factor(Propagule_pressure))) +  geom_point(size=2) + 
    xlab("Propagule richness") +ylab("Success") + theme_classic() + 
    theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+ 
    theme(panel.border= element_blank()) + 
    theme(axis.line.x = element_line(color="black", size = 1), axis.line.y = element_line(color="black", size = 1))+
    labs(color='Propagule Pressure') # +  geom_ribbon(aes(ymin = Min, ymax = Max,fill=as.factor(Home_popsize)), alpha = 0.5) + guides(fill = FALSE) 
...
ProportionPlots <- function(Invador_popsize,Invador_generations,Mutation_rate,Selection_strength,Home_popsize){
  Propagule_richness = unique(finaldf$Propagule_richness)[[1]]
  Popsize_Generations = finaldf[which(finaldf$Mutation_rate == Mutation_rate &finaldf$Selection_strength == Selection_strength & finaldf$Propagule_pressure == Propagule_pressure & finaldf$Home_popsize == Home_popsize & finaldf$Propagule_richness == Propagule_richness),]
  Popsize_PropagulePressure = finaldf[which(finaldf$Mutation_rate == Mutation_rate &finaldf$Selection_strength == Selection_strength & finaldf$Invador_generations == Invador_generations & finaldf$Home_popsize == Home_popsize & finaldf$Propagule_richness == Propagule_richness),]
  Generations_PropagulePressure = finaldf[which(finaldf$Mutation_rate == Mutation_rate &finaldf$Selection_strength == Selection_strength & finaldf$Invador_popsize == Invador_popsize & finaldf$Home_popsize == Home_popsize & finaldf$Propagule_richness == Propagule_richness),]
  p1 <-  ggplot(Popsize_Generations,aes(x=as.factor(Invador_generations), y=as.factor(Invador_popsize))) +  geom_tile(aes(fill=Proportion)) +
    xlab(expression( "Invader Generations g"[r]) ) +ylab(expression( "Invader Metacommunity Size J"[r]) ) + theme_classic() + theme(panel.border= element_blank()) + 
  ...
  p2 <-  ggplot(Popsize_PropagulePressure,aes(x=as.factor(Propagule_pressure), y=as.factor(Invador_popsize))) +  geom_tile(aes(fill=Proportion)) +
    xlab("Propagule Pressure p") +ylab(expression( "Invader Metacommunity Size J"[r]) ) + theme_classic() + theme(panel.border= element_blank()) + 
    theme(text = element_text(size=18),plot.background= element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank()) +
    scale_fill_gradient2(low = "yellow", mid = "red", high = "blue",midpoint = 0.5, limits = c(0,1)) + guides(fill=FALSE) +
    labs(fill='Proportion') + theme(legend.text = element_text(size=18),legend.key.size = unit(1, "cm"))+theme(axis.title=element_text(size=24))
  ...
  Propagule_richness = unique(finaldf$Propagule_richness)[[2]]
  Popsize_Generations = finaldf[which(finaldf$Mutation_rate == Mutation_rate &finaldf$Selection_strength == Selection_strength & finaldf$Propagule_pressure == Propagule_pressure & finaldf$Home_popsize == Home_popsize & finaldf$Propagule_richness == Propagule_richness),]
  Popsize_PropagulePressure = finaldf[which(finaldf$Mutation_rate == Mutation_rate &finaldf$Selection_strength == Selection_strength & finaldf$Invador_generations == Invador_generations & finaldf$Home_popsize == Home_popsize & finaldf$Propagule_richness == Propagule_richness),]
  Generations_PropagulePressure = finaldf[which(finaldf$Mutation_rate == Mutation_rate &finaldf$Selection_strength == Selection_strength & finaldf$Invador_popsize == Invador_popsize & finaldf$Home_popsize == Home_popsize & finaldf$Propagule_richness == Propagule_richness),]  
  ...
  Propagule_richness = unique(finaldf$Propagule_richness)[[3]]
  Popsize_Generations = finaldf[which(finaldf$Mutation_rate == Mutation_rate &finaldf$Selection_strength == Selection_strength & finaldf$Propagule_pressure == Propagule_pressure & finaldf$Home_popsize == Home_popsize & finaldf$Propagule_richness == Propagule_richness),]
  Popsize_PropagulePressure = finaldf[which(finaldf$Mutation_rate == Mutation_rate &finaldf$Selection_strength == Selection_strength & finaldf$Invador_generations == Invador_generations & finaldf$Home_popsize == Home_popsize & finaldf$Propagule_richness == Propagule_richness),]
  Generations_PropagulePressure = finaldf[which(finaldf$Mutation_rate == Mutation_rate &finaldf$Selection_strength == Selection_strength & finaldf$Invador_popsize == Invador_popsize & finaldf$Home_popsize == Home_popsize & finaldf$Propagule_richness == Propagule_richness),]  
  ...
  a <-  arrangeGrob(p1, left = textGrob("a)", x = unit(0, "npc"),
                    ...
                    top = textGrob(bquote("r" == .(unique(finaldf$Propagule_richness)[1])),
  ...
  d <-  arrangeGrob(p4, left = textGrob("d)", x = unit(0, "npc"),
                                        y   = unit(0.95, "npc"), just=c("left","top"),
                                        gp=gpar(col="black", fontsize=24)),
                    top = textGrob(bquote("r" == .(unique(finaldf$Propagule_richness)[2])),
                                   gp=gpar(col="black", fontsize=24,fontface="bold")))
  ...
  g <-  arrangeGrob(p7, left = textGrob("g)", x = unit(0, "npc"),
                                        y   = unit(0.95, "npc"), just=c("left","top"),
                                        gp=gpar(col="black", fontsize=24)),
                    top = textGrob(bquote("r" == .(unique(finaldf$Propagule_richness)[3])),
                                   gp=gpar(col="black", fontsize=24,fontface="bold")))
  ...
  pdf(paste("../../Plots/InvasionModel/TypeProportionPlots.pdf"),height=18.5,width=22)
...
SuccessPlots <- function(Invador_popsize,Invador_generations,Mutation_rate,Selection_strength,Home_popsize){
  Propagule_richness = unique(finaldf$Propagule_richness)[[1]]
  Popsize_Generations = finaldf[which(finaldf$Mutation_rate == Mutation_rate &finaldf$Selection_strength == Selection_strength & finaldf$Propagule_pressure == Propagule_pressure & finaldf$Home_popsize == Home_popsize & finaldf$Propagule_richness == Propagule_richness),]
  Popsize_PropagulePressure = finaldf[which(finaldf$Mutation_rate == Mutation_rate &finaldf$Selection_strength == Selection_strength & finaldf$Invador_generations == Invador_generations & finaldf$Home_popsize == Home_popsize & finaldf$Propagule_richness == Propagule_richness),]
  Generations_PropagulePressure = finaldf[which(finaldf$Mutation_rate == Mutation_rate &finaldf$Selection_strength == Selection_strength & finaldf$Invador_popsize == Invador_popsize & finaldf$Home_popsize == Home_popsize & finaldf$Propagule_richness == Propagule_richness),]   
  ...
  lay = rbind(c(1,1,1,2,2,2,3,3,3,NA),c(1,1,1,2,2,2,3,3,3,NA),c(1,1,1,2,2,2,3,3,3,NA),
              c(4,4,4,5,5,5,6,6,6,10),c(4,4,4,5,5,5,6,6,6,10),c(4,4,4,5,5,5,6,6,6,10),
              c(7,7,7,8,8,8,9,9,9,NA),c(7,7,7,8,8,8,9,9,9,NA),c(7,7,7,8,8,8,9,9,9,NA))
  pdf(paste("../../Plots/InvasionModel/TypeSuccessPlots.pdf"),height=18.5,width=22)
  grid.arrange(a,d,g,b,e,h,c,f,i,legend,layout_matrix = lay,top = "")
  dev.off()
  return()
}
...
# paramatersdf <- suppressWarnings(loadparamaters(360,100))
# print("1 done")
...
# save(finaldf,file="TypeInvasionData.rdata")
load(file ="TypeInvasionData.rdata")
...
Propagule_richness = 3
SuccessPlots(Invador_popsize,Invador_generations,Mutation_rate,Selection_strength,Home_popsize)
ProportionPlots(Invador_popsize,Invador_generations,Mutation_rate,Selection_strength,Home_popsize)
