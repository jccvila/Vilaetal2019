...
# save(plotdata,file = 'Multiplicative_Data.rds')
# load('Multiplicative_Data.rds')
# # #
...
# save(plotdata,file = 'Multiplicative_Data3.rds')
# stop()
load('Multiplicative_Data3.rds')
s = unique(plotdata$selection_strength)
u = unique(plotdata$mutation_rate)
p = unique(plotdata$propagule_pressure)
i <- unique(plotdata$invaded_popsize)
...
Time_series_plot <- function(){
  ...
  ggsave(file = '../../Plots/Timeseries_M.png', grid.arrange(arrangeGrob(p1),arrangeGrob(p2),arrangeGrob(p3),arrangeGrob(p4),arrangeGrob(p5),arrangeGrob(p6),arrangeGrob(p7),
...
Evolution_Plots <- function(a,b,d){
  ...
  ggsave(file = '../../Plots/Evolution_M.png',grid.arrange(p1,p2,p3,p4,leg
...
Invader_Plots <- function(a,b,d){
  ...
  p4 <- ggplot(sum_df4, aes(x = Relative_Propagule_Fitness,y = mean)) +  geom_point(size=2) + 
    ...
    labs(color='Invaded Community Size') + 
    theme(text = element_text(size=18),legend.text = element_text(size=15),legend.key.size = unit(0.5, "cm")) 
  ggsave(file = '../../Plots/Invaders_M.png',grid.arrange(arrangeGrob(p1),arrangeGrob(p2),arrangeGrob(p3),arrangeGrob(p4)
...
Invaded_Plots <- function(a,b,d){
  ...
  ggsave(file = '../../Plots/Invaded_M.png',grid.arrange(arrangeGrob(p1),arrangeGrob(p2),arrangeGrob(p3),arrangeGrob(p4)
                                                         ,layout_matrix = rbind(c(1,2),c(3,4))),width = 12, height = 10)
...
Recreation_plots <- function(){
  ...
  ggsave(file = '../../Plots/Recreation_M.png',grid.arrange(arrangeGrob(p1a,top=textGrob('A)',x=0.05,gp=gpar(fontsize=15))),
